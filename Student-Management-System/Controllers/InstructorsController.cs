...
namespace Student_Management_System.Controllers
{
    public class InstructorsController : Controller
    {
        ...
        {
            var Courses = await _context.CourseAssignments.Where(x => x.InstructorID == id).Include(x => x.Course)
                .ThenInclude(x => x.Department).ToListAsync();
            return PartialView(Courses.Select(x => x.Course));
        }
        public async Task<IActionResult> _Index2(int courseID)
        {
            return PartialView(await _context.Enrollments.Include(x => x.Student).Where(x => x.CourseID == courseID).ToListAsync());
        }
